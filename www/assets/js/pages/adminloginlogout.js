...
function loginLogoutadmin() {
    if (FYSCloud.Session.get('userId')) {
        $('#navbarCollapse').append(
            `<a href='adminpagina.html' class='text-white text-uppercase'><b>admin</b></a>
...
function accountgegevensVerwijder() {
    let voornaam = $("#voornaam").val();
    let achternaam = $("#achternaam").val();

    FYSCloud.API.queryDatabase(
        "DELETE * FROM gebruiker WHERE = ? AND = ?",
        [voornaam,achternaam]
    ).done(function (data) {
        console.log(data[0]);
...
//hier wordt data opgehaald
function dataladen(){
        FYSCloud.API.queryDatabase(
            "SELECT * FROM gebruiker WHERE rechten_id = ?",
            [1]
        ).done(function(data) {
            //hier wordt een table gemaakt
            let myStringArray = data;
            let arraylength = myStringArray.length;
            var perrow = 1, // 1 items per row dont change pls
                html = "<table><th>gebruiker_id</th><th>voornaam</th><th>tussenvoegsel</th><th>achternaam</th>" +
                    "<th>geboortedatum</th><tr>";
            for(let i=0; i<arraylength; i++){
                html += "<td>" + myStringArray[i]["gebruiker_id"] + "</td>" +
                    "<td>" + myStringArray[i]["voornaam"] + "</td>" +
                "<td>" + myStringArray[i]["tussenvoegsel"] + "</td>" +
                "<td>" + myStringArray[i]["achternaam"] + "</td>" +
                    "<td>" + myStringArray[i]["geboortedatum"].replace("T00:00:00.000Z","") + "</td>";
                // Break into next row
                var next = i+1;
                if (next%perrow===0 && next!==arraylength) {
                    html += "</tr><tr>";
                }
                console.log(myStringArray[i]["voornaam"]);
            }
            document.getElementById("gebruikertabel").innerHTML = html;
        }).fail(function (reason) {
            console.log(reason);
        })
}